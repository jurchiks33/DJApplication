      ...
      bypassButton("Bypass"), resetButton("Reset")
{
    ...
    // Setup Bypass button with toggle and color states
    bypassButton.setClickingTogglesState(true);
    bypassButton.setButtonText("Bypass");
    bypassButton.addListener(this);
    bypassButton.setColour(juce::TextButton::buttonOnColourId, juce::Colours::lightcoral); // Active state color
    bypassButton.setColour(juce::TextButton::buttonColourId, juce::Colours::lightgreen);   // Inactive state color
    addAndMakeVisible(bypassButton);
...
void EqualizerComponent::buttonClicked(juce::Button* button)
{
    ...
    {
        isBypassed = bypassButton.getToggleState();
        updateBypassButtonColor(); // Update the button color
        std::cout << (isBypassed ? "EQ Bypassed" : "EQ Enabled") << std::endl;
    }
    else if (button == &resetButton)
    {
...
void EqualizerComponent::updateBypassButtonColor()
{
    if (isBypassed)
    {
        bypassButton.setColour(juce::TextButton::buttonColourId, juce::Colours::lightcoral);  // Light red when active
    }
    else
    {
        bypassButton.setColour(juce::TextButton::buttonColourId, juce::Colours::lightgreen);  // Light green when inactive
    }
}
